...
def get_nearest_neighbors(E, estimate_vector):
    # Preconvert the estimate_vector to floats once
    float_estimates = list(map(float, estimate_vector))
    
    # Use heapq.nsmallest to find the 6 smallest distances
    nearest_neighbors = heapq.nsmallest(6, ((abs(E - val) ** 2, idx) for idx, val in enumerate(float_estimates)))
...
#      # check the length
#      print(f"length of HILL: {len(hll)}")
